...
def main() -> None:
    ...
    pairs = tuple(v.strip() for v in opts.variants.split(',') if v.strip())
    if not pairs or set(pairs) - set(PAIRS):
        sys.exit(f'--variants takes {", ".join(PAIRS)}')
    if set(pairs) != set(PAIRS) and out == OUT.resolve():
    ...
    def on_spec(spec: dict, results: dict) -> None:
        ...
        for n in TARGETS:
            ...
            for v in variants(spec, n, pairs):
    ...
    tasks = [Task(s, n, v) for s in queue for n in TARGETS for v in variants(s, n, pairs)]
    if not tasks:
        sys.exit(f'nothing to run: no queued spec has the {", ".join(pairs)} variants')
